...
(function () {
  ...
  function mount(){
    ...
    var wantBar = body.getAttribute('data-pimc-shell-bar') !== 'off';
    var skip = el('a', { 'class':'pds-skip', href:'#pshell-main' }, 'Skip to main content');
    var bar = wantBar ? buildBar({ title:title }) : null;
    var nav = buildNav(curr);
    body.classList.add('pshell-on');
    if (!wantBar) body.classList.add('pshell-nav-only');
    ...
    if (bar) body.insertBefore(bar, body.firstChild);   // top bar first (optional)
